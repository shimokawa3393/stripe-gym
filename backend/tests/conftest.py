...
def client(db_engine):
    ...
    with app.test_client() as client:
        with app.app_context():
            # 各テスト関数実行前端ーベースをクリーンな状態にする
            # 外部キー制約があるため、子テーブ順序で削除
            session = get_session()
            try:
                # テーブルの存在に依存してクリーンアップ（存在しないテーブルエラーは無視）
                if session.bind.dialect.has_table(session.bind, 'ledger'):
                    session.query(Ledger).delete()
                if session.bind.dialect.has_table(session.bind, 'subscription'):
                    session.query(Subscription).delete()
                if session.bind.dialect.has_table(session.bind, 'processed_events'):
                    session.query(ProcessedEvent).delete()
                if session.bind.dialect.has_table(session.bind, 'users'):
                    session.query(User).delete()
                session.commit()
            except Exception as e:
                session.rollback()
                # DBクリーンアップエラーは正常なケースもあるので無視
                print(f"テストクリーンアップ（無視可能）: {type(e).__name__}")
            finally:
                session.close()
        yield client
